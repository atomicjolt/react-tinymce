...
const TinyMCE = React.createClass({
  ...
  componentWillReceiveProps(nextProps) {
    if (!isEqual(this.props.config, nextProps.config, compareConfig)) {
  ...
  shouldComponentUpdate(nextProps) {
    return (
      ...
      !isEqual(this.props.config, nextProps.config, compareConfig)
    );
  },
  ...
  _init(propsConfig, content) {
    const config = cloneDeep(propsConfig);

    if (this._isInit) {
      this._remove();

      // When rapidly switching editors in and out, the toolbar may not always
      // be removed if it's in a fixed container
      if (this.props.config.fixed_toolbar_container) {
        const oldId = this.props.config.fixed_toolbar_container.substr(1);
        const newId = config.fixed_toolbar_container.substr(1);
        const toolbar = document.getElementById(oldId) || document.getElementById(newId);
        toolbar.innerHTML = '';
      }
    }
